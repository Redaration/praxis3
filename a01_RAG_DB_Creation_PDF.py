...
def setup_chroma_db(collection_mode="reset"):
    """
    ...
    if collection_mode == "reset":
        ...
        # Append to existing collection or create if it does not exist
        ...
        if existing_count:
            print(f"Appending to existing collection: {collection_name} ({existing_count} chunks already stored)")
        else:
            print(f"Created new collection: {collection_name}")
    
    return chroma_client, collection

def add_documents_to_chroma(collection, documents, append_mode=False):
    """
    ...
    Args:
        ...
        append_mode: If True, retain existing data and update overlapping IDs
    """
    if not documents:
    ...
    use_upsert = append_mode and hasattr(collection, "upsert")
    if append_mode:
        if use_upsert:
            print("Append mode active: using upsert to merge document chunks.")
        else:
            print("Append mode active: removing overlapping chunk IDs before adding (upsert not available).")
    ...
    for i in range(0, len(texts), batch_size):
        ...
        batch_docs = texts[i:end_idx]
        batch_ids = ids[i:end_idx]
        batch_metadatas = metadatas[i:end_idx]
        
        if append_mode and not use_upsert:
            try:
                collection.delete(ids=batch_ids)
            except Exception as e:
                print(f"    Warning: could not remove existing IDs before append: {e}")
        
        target_fn = collection.upsert if use_upsert else collection.add
        target_fn(
...
def parse_args():
    parser = argparse.ArgumentParser(description="Create or update the Cisco AI PDF RAG database.")
    parser.add_argument(
        "--collection-mode",
        choices=["reset", "append"],
        default=None,
        help="Choose whether to reset (delete) the existing collection or append to it. If omitted, you will be prompted."
    )
    return parser.parse_args()

def prompt_collection_mode():
    """Prompt the user to choose whether to reset or append the collection."""
    while True:
        response = input("\nClear the existing Chroma collection (y) or continue adding data to the collection (n)? [y/N]: ").strip().lower()
...
def main():
    ...
    args = parse_args()
    collection_mode = args.collection_mode or prompt_collection_mode()
    
    print("="*80)
    print("Cisco AI PDF RAG Database Creator")
    print("="*80)
    print(f"Collection mode: {collection_mode.upper()}")
    
    # Process PDF files
    documents = process_pdf_files()
    
    # Setup ChromaDB
    chroma_client, collection = setup_chroma_db(collection_mode=collection_mode)
    
    # Add documents to ChromaDB
    add_documents_to_chroma(collection, documents, append_mode=(collection_mode == "append"))
